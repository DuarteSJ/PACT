...
/*
 * mm/internal.h is a kernel-private header (not exported to module builds),
 * so we include it by relative path into the kernel source tree. The tree
 * is provided at build time via the Makefile's KERNEL_SRC variable
 * (default: /lib/modules/$(uname -r)/build). See the Makefile and the
 * nomad README for how to point this at your linux-5.13-rc6 source.
 */
